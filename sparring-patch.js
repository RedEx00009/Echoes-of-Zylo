...
(function patchSparring() {
    ...
    function main() {
      ...
      var MOVES = {
        sparring: { dmgBase:0.08, dmgVar:0.04, ki:5,  label:"SPARRING!",    atk:"light_combo", color:"#f5c400" },
        ki_clash: { dmgBase:0.13, dmgVar:0.05, ki:20, label:"KI CLASH!!",   atk:"charge",      color:"#00e5ff" },
        gut_hit:  { dmgBase:0.11, dmgVar:0.03, ki:8,  label:"¡ESTOMAGO!",  atk:"light_combo", color:"#ff7043" },
        face_hit: { dmgBase:0.10, dmgVar:0.06, ki:6,  label:"¡EN LA CARA!", atk:"heavy_combo", color:"#ff5252" },
      };
      var SELF_HIT = new Set(["take_gut", "take_face", "lose"]);
  
      // Duración aproximada de cada animación en ms
      var ANIM_DUR = {
        light_combo: 750, heavy_combo: 550, special: 800, charge: 700,
        hit: 850, knockback: 900, dash_rush: 500, block: 400, recovery: 600,
        fly_combat: 300, combat_idle: 200,
        attack_1: 750, attack_2: 500, ki_charge: 700, hurt: 850, death: 1500, jump: 600,
      };

      var COMBAT_ANIMS = new Set([
        "light_combo", "heavy_combo", "special", "charge", "hit", "knockback",
        "dash_rush", "block", "recovery", "fly_combat",
        "attack_1", "attack_2", "ki_charge", "hurt", "death", "jump",
      ]);
      ...
      function initCombatAnimsListener() {
        ...
        function handleCombatAnim(snap) {
          ...
          var endAnim = remoteAnim.battleMode ? "combat_idle" : "idle";
          if (proto && proto.play) {
            proto.play.call(remoteAnim, animName, function () {
              animProtect[ownerId] = 0;
              proto.play.call(remoteAnim, endAnim);
            });
          } else {
            remoteAnim.play(animName, function () {
              animProtect[ownerId] = 0;
              remoteAnim.play(endAnim);
            });
          }
          ...
          if (remotePlayer) {
            ...
            if (animName === "death" || animName === "knockback") {
      ...
      function takeDamage(dmg, moveId, fromName) {
        ...
        playMine("hit", function () {
          if (!koInProgress) playMine(G.flying ? (G.myAnim && G.myAnim.battleMode ? "fly_combat" : "fly") : (G.myAnim && G.myAnim.battleMode ? "combat_idle" : "idle"));
        });
  
        // Publicar en combatAnims/ → todos ven mi hit
        writeCombatAnim("hit", {
          hp:    p.hp,
          dmg:   dmg,
          label: "-" + dmg,
      ...
      function ko(fromName) {
        ...
        if (G.online && G.myId) {
          ...
          G.db.ref("players/" + G.myId).update({ hp: 0, animAction: "knockback", t: Date.now() });
        }
        ...
        setTimeout(function () {
          ...
          myLockAnim   = "idle";
          hideKO();
          playMine(G.myAnim && G.myAnim.battleMode ? "combat_idle" : "idle");
          if (typeof window.updatePlayerHud === "function") window.updatePlayerHud();
          if (G.online && G.myId) {
      ...
      function hookIncoming() {
        ...
        G.db.ref("partyActions/" + G.myId).on("value", function (snap) {
          ...
          var animName = act.anim || "hit";
          playMine(animName, function () {
            playMine(G.flying ? (G.myAnim && G.myAnim.battleMode ? "fly_combat" : "fly") : (G.myAnim && G.myAnim.battleMode ? "combat_idle" : "idle"));
          });
      ...
      function playMine(anim, cb) {
        if (!G.myAnim) return;
        if (G.myAnim.setBattleMode) G.myAnim.setBattleMode(true);
        if (typeof window.setControlMode === "function" && !window.isCombatMode) {
          window.setControlMode("combat");
        }
        var CS = window.CharacterSystem;
        var resolved = (CS && CS.resolveCombatAction)
          ? CS.resolveCombatAction(anim, true)
          : anim;
        G.myAnim.play(resolved, cb || null);
      }
      ...
      function injectSurrender() {
        ...
        btn.onclick = function () {
          ...
          playMine("hit", function () { playMine(G.myAnim && G.myAnim.battleMode ? "combat_idle" : "idle"); });
          writeCombatAnim("hit", { label: "RENDIDO", color: "#90a4ae" });
